...
function HeroBadges({ compact = false }: { compact?: boolean }) {
  return (
    <div
      className={
        compact
      ...
      {FLOATING_BADGES.map((badge) => {
        ...
        return (
          <span
            ...
            className={
              compact
                ? "inline-flex items-center gap-1.5 rounded-full border border-[#C5D9EB]/80 bg-white/95 px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm"
  ...
  if (variant === "compact") {
    return (
      <motion.div
        ...
        className="w-full"
      >
        <div className="relative h-36 overflow-hidden rounded-[24px] shadow-[0_12px_32px_rgba(52,89,126,0.14)] ring-1 ring-white/70 sm:h-40">
          <LoginHeroVisual
            priority
            sizes="100vw"
          ...
          />
          <div className="absolute inset-x-0 bottom-0 bg-linear-to-t from-white/95 via-white/55 to-transparent px-4 pb-4 pt-10">
            <HeroCopy compact />
          </div>
        </div>
        <div className="mt-3">
