...
export function AdminSidebarNav({
  ...
  return (
    <>
      <div className={compactLogo ? "px-4 pt-5" : "px-5 pt-7"}>
        <Link
          href="/app/admin"
          className="block rounded-2xl p-1 transition hover:bg-[#EEF4FA]"
          onClick={onNavigate}
        >
          <Image
            src="/logo_main.svg"
            ...
            width={168}
            height={40}
            className={compactLogo ? "h-7 w-auto" : "h-9 w-auto"}
            priority
          />
      ...
      <nav
        className={`flex flex-1 flex-col gap-1 ${compactLogo ? "mt-6 px-2" : "mt-8 px-3"}`}
