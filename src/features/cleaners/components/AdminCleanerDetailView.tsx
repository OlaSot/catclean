...
export default function AdminCleanerDetailView({ cleanerId }: { cleanerId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!cleanerRes.ok || cleanerJson.error || !cleanerJson.data) {
        setError(cleanerJson.error ?? "Failed to load cleaner");
        setCleaner(null);
        return;
      }
  ...
  async function saveSettings() {
    ...
    try {
      ...
      if (!response.ok || json.error || !json.data) {
        setError(json.error ?? "Failed to save cleaner settings");
        return;
      }
      setCleaner(json.data);
    } catch {
      setError("Failed to save cleaner settings");
    } finally {
  ...
  async function upsertAvailability() {
    ...
    try {
      ...
      if (!response.ok || json.error) {
        setError(json.error ?? "Failed to save availability");
        return;
      }
      await load();
    } catch {
  ...
  if (loading) {
    return <div className="rounded-3xl border border-slate-200 bg-white p-6 text-sm text-slate-500">Loading cleaner...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-3xl border border-slate-200/80 bg-white p-5">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Workload settings</h2>
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <label className="text-sm text-slate-600 sm:col-span-2">
            Preferred work cities (comma separated)
      ...
      <section className="rounded-3xl border border-slate-200/80 bg-white p-5">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          ...
          <input value={availabilityNote} onChange={(e) => setAvailabilityNote(e.target.value)} placeholder="Note" className="rounded-xl border border-slate-200 px-3 py-2 text-sm" />
        </div>
        ...
        <div className="mt-4 space-y-2">
          {availability.length === 0 ? (
            <p className="text-sm text-slate-500">No availability records in selected range.</p>
          ) : (
